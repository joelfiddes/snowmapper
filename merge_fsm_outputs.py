...
def concat_fsm_with_overwrite(mydir):
    ...
    for myname in tqdm(filenames, desc="Concatenating FSM files"):
    ...
    if logger:
        logger.info(f"Concatenated and saved {saved_count} FSM files")
...
def simulate_fsm(mydir):
    ...
    - mydir (str): Directory containing the simulation outputs.
    """
    os.chdir(os.path.join(mydir, "outputs"))
    fsm_files = glob.glob("./FSM*")
...
def main(mydir):
    ...
    logger = setup_logger_with_tqdm("concat_fsm", file=False)
    ...
    simulate_fsm(mydir)
    logger.info("FSM simulation complete")
